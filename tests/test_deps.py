...
class DepsTestCase(unittest.TestCase):
    ...
    def test_relationship_parsing_single_relationship(self):
        expected = deps.AndRelationships(
            relationships=(deps.VersionedRelationship(name="foo", operator="=", version="1.0"),)
        )
    ...
    def test_parse_alternatives(self):
        ...
        expected = deps.OrRelationships(
            relationships=(deps.Relationship(name="python2.6"), deps.Relationship(name="python2.7"))
        )
        assert depends == expected
